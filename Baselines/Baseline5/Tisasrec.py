...
class TimeAwareMultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, queries, keys, time_mask, attn_mask, time_matrix_K, time_matrix_V, abs_pos_K, abs_pos_V):
        # time_mask: padding item的mask,     attn_mask: causality的mask,下三角
...
class TiSASRec(torch.nn.Module): # similar to torch.nn.MultiheadAttention
    def __init__(self, user_num, item_num, args):
        ...
        self.item_emb = torch.nn.Embedding(self.item_num+1, args.hidden_units, padding_idx=0)   # ---公式（3）
        self.item_emb_dropout = torch.nn.Dropout(p=args.dropout_rate)

        self.abs_pos_K_emb = torch.nn.Embedding(args.maxlen, args.hidden_units)                 # ---公式（3）
        self.abs_pos_V_emb = torch.nn.Embedding(args.maxlen, args.hidden_units)                 # ---公式（4）
        self.time_matrix_K_emb = torch.nn.Embedding(args.time_span+1, args.hidden_units)        # ---公式（5）
        self.time_matrix_V_emb = torch.nn.Embedding(args.time_span+1, args.hidden_units)        # ---公式（5）
    ...
    def seq2feats(self, user_ids, log_seqs, time_matrices):
        ...
        for i in range(len(self.attention_layers)):
            ...
            Q = self.attention_layernorms[i](seqs) # PyTorch mha(Multi-Head self-Attention) requires time first fmt
            mha_outputs = self.attention_layers[i](Q, seqs,
                                            timeline_mask, attention_mask,
                                            time_matrix_K, time_matrix_V,
                                            abs_pos_K, abs_pos_V)   # 做完时间multi-head self-attention的输出
    ...
    def forward(self, user_ids, log_seqs, time_matrices, pos_seqs, neg_seqs): # for training
        ...
        pos_logits = (log_feats * pos_embs).sum(dim=-1)     # 正例的logits得分
        neg_logits = (log_feats * neg_embs).sum(dim=-1)     # 负例的logits得分
